...
import CnRareDiseasePopup from "./CnRareDiseasePopup";

import styles from "./RareDisease.less";
let underscore = require("underscore");
import debounce from "lodash/debounce";

@connect(({ cnRareDisease }) => ({
  cnRareDisease
}))
export default class CnRareDiseaseList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      keyword: "",
  ...
  statusRender(text, record, index) {
    ...
    let comp = <span style={{ color: color, fontSize: 18 }}>{iconA}</span>;
    return comp;
  }

  render() {
    const { data, total } = this.props.cnRareDisease;
