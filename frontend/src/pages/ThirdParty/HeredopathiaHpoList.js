...
import HeredopathiaHpoPopup from "./HeredopathiaHpoPopup";

import styles from "./ThirdParty.less";
import ImpHeredopathiaHpos from "./ImpHeredopathiaHpos";
let underscore = require("underscore");

@connect(({ heredopathiaHpo }) => ({
  heredopathiaHpo
}))
export default class HeredopathiaHpoList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      page: 1,
  ...
  render() {
    const { data, total } = this.props.heredopathiaHpo;
    ...
    const columns = [
      ...
      {
        dataIndex: "phenotype_cn",
        title: "表型名称",
        render: this.longTextRender.bind(this)
      },
      { dataIndex: "symbols", title: "遗传病" },
