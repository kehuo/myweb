...
import TrackLogPopup from "./TrackLogPopup";
...
export default class TrackLogList extends ElementComponent {
  constructor(props) {
    super(props);
    let now = moment();
  ...
  buildOpBar() {
    const { orgOpts, deptOpts } = this.props.trackLog;
    const {
      org,
      operator,
      department,
  ...
  render() {
    const { data, total, detailOne } = this.props.trackLog;
